...
from tensorflow.keras.layers import Layer  # 你写的自定义类是继承自 Layer 的
...
class LearnableCLSTokens(tf.keras.layers.Layer):
    def __init__(self, data_types, dim, **kwargs):
        super().__init__(**kwargs)
        ...
        self.yellow_token = self.add_weight(
            name='yellow_cls_token',
            shape=(1, 1, dim),
            initializer='random_normal',
            trainable=True
        )
    ...
    def call(self, batch_size):
        tokens = {k: tf.tile(v, [batch_size, 1, 1]) for k, v in self.cls_tokens.items()}
        yellow = tf.tile(self.yellow_token, [batch_size, 1, 1])
